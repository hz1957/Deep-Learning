...
class SaliencyMap:
    def compute_saliency_maps(self, X, y, model):
        ...
        pred = model(X_var)
        true_label = y_var
        correct_class_score = pred.gather(1, true_label.view(-1, 1)).squeeze()


        # 2. Backpropagation
        torch.sum(correct_class_score).backward(retain_graph=True)
        
        saliency, _ = torch.max(X_var.grad.data.abs(), dim = 1)
