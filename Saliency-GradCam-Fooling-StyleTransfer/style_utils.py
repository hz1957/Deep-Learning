...
def style_transfer(name, content_image, style_image, image_size, style_size, content_layer, content_weight,
    ...
    for t in range(200):
        ...
        #                                                                            #
        # HINTS: all the weights and loss functions are defined. You don't need to   #
        # add any other extra weights for the three loss terms.                      #
        ...
        ##############################################################################
        current_content_loss = content_loss(content_weight, feats[content_layer], content_target)
        current_style_loss = style_loss(feats, style_layers, style_targets, style_weights)
        current_tv_loss = tv_loss(img_var, tv_weight)

        current_loss = current_content_loss + current_style_loss + current_tv_loss
