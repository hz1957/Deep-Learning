...
def generate_batched_data(data, label, batch_size=32, shuffle=False, seed=None):
    ...
    # Shuffle if required
    if shuffle:
        combined_data = list(zip(data, label))
        random.shuffle(combined_data)
        data, label =zip(*combined_data)
    
    # Generate batches of images with the required batch size 
    num_batches = len(data)//batch_size
    batched_data = [np.array(data[i*batch_size:(i+1)*batch_size]) for i in range(num_batches)]
    batched_label = [np.array(label[i*batch_size:(i+1)*batch_size]) for i in range(num_batches)]
    
    # Append the last batch if it exists
    if len(data) % batch_size != 0:
        batched_data.append(np.array(data[num_batches*batch_size:]))
        batched_label.append(np.array(label[num_batches*batch_size:]))
