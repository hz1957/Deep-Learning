...
class _baseNetwork:
    ...
    def softmax(self, scores):
        ...
        #############################################################################
        # Use -np.max to avoid large numbers to avoid instability
    ...
    def cross_entropy_loss(self, x_pred, y):
        ...
        #############################################################################
        N = len(x_pred)
        # For each entry, calculate the product of indicator and log(x_pred)
        loss_x = - np.log(x_pred[np.arange(N),y])
        # Then average across all entries
        loss = np.sum(loss_x) / N
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
        return loss
    ...
    def compute_accuracy(self, x_pred, y):
        ...
        #############################################################################
        N = len(x_pred)
        x_pred_label = np.argmax(x_pred, axis=1)
        acc = np.sum(x_pred_label == y) / N
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
        return acc
