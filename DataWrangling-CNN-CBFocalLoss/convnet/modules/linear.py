...
class Linear:
    ...
    def forward(self, x):
        ...
        #############################################################################
        N = x.shape[0]
        x_flattend = x.reshape((N, self.in_dim))

        out = x_flattend.dot(self.weight) + self.bias
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
        self.cache = x
    ...
    def backward(self, dout):
        ...
        #############################################################################
        N = x.shape[0]
        x_flattened = x.reshape((N, self.in_dim))

        self.dw = (x_flattened.T.dot(dout)).reshape(self.weight.shape)
        self.db = dout.sum(axis = 0, keepdims = True)
        self.dx = self.weight.dot(dout.T).T.reshape(x.shape)
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
